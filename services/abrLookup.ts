...
function cleanAbn(value: unknown): string {
...
async function fetchAbrJson(url: string): Promise<any | null> {
  try {
...
function extractCandidates(payload: any): any[] {
  ...
  const abnValue =
    payload.Abn || payload.ABN || payload.AbnNumber || payload.AbnIdentifier || payload.abn;
  if (abnValue) return [payload];
  return [];
}

function toLookupResult(candidate: any): AbrLookupResult | null {
...
async function queryAbrByName(name: string, state?: string | null): Promise<AbrLookupResult | null> {
  const url = new URL(BASE_URL);
  url.searchParams.set('guid', process.env.ABR_KEY as string);
...
export async function lookupABN(input: {
  businessName: string;
