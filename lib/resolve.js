...
function pickBest(candidates = {}, targetKey = '', { min = 0.68 } = {}) {
  ...
  for (const [key, val] of Object.entries(candidates)) {
    ...
    if (val && typeof val === 'object' && Array.isArray(val.aliases)) {
      aliases.push(...val.aliases);
    }
    for (const alias of aliases) {
      const score = similar(alias, targetKey);
      if (score > bestScore) {
        bestKey = key;
        bestScore = score;
        bestVal = val;
...
function collectTextEntries(entries = []) {
...
function resolveServiceInclusions(blocks = []) {
  const inclusionRegex = /(includes|features|what you get|benefits):?\s*(.+)/i;
  const texts = collectTextEntries(blocks);
  return texts
    .map((text) => {
      const match = text.match(inclusionRegex);
...
function extractServicePrice(text = '') {
  const pricePattern = /(?:\$|aud|\$aud)?\s*(\d{2,5}(?:,\d{3})?(?:\.\d+)?)/i;
...
function extractPriceNote(text = '') {
  ...
  if (/\bper\s*hour\b|\b\/\s*hour\b|\bhr\b/.test(lower)) return 'per hour';
  if (/\bper\s*day\b|\b\/\s*day\b/.test(lower)) return 'per day';
  if (/\bper\s*week\b|\bweekly\b|\b\/\s*week\b/.test(lower)) return 'per week';
  if (/\bper\s*month\b|\bmonthly\b|\b\/\s*month\b/.test(lower)) return 'per month';
  if (/\bper\s*year\b|\bannual\b|\byearly\b|\b\/\s*year\b/.test(lower)) return 'per year';
  if (/\bper\s*visit\b|\b\/\s*visit\b/.test(lower)) return 'per visit';
  if (/\bper\s*service\b|\b\/\s*service\b/.test(lower)) return 'per service';
  if (/\bper\s*job\b|\b\/\s*job\b/.test(lower)) return 'per job';
  return null;
}

function inferDeliveryMode(text = '') {
  if (/membership|plan|subscription/i.test(text)) return { value: 'Subscription', confidence: 0.85 };
  if (/on[-\s]?site|visit|callout|on site/i.test(text)) return { value: 'On-site', confidence: 0.8 };
  return { value: 'Standard Service', confidence: 0.6 };
}

function resolveServiceDetails(raw = {}) {
  const headings = collectTextEntries(raw.headings);
  const textBlocks = collectTextEntries(raw.text_blocks);
  const inclusionSource = textBlocks.length ? textBlocks : headings;

  const inclusions = resolveServiceInclusions(inclusionSource);
  const inclusionConf = inclusions.map(() => 0.78);

  const priceDetails = headings
    .map((text) => {
      const price = extractServicePrice(text);
      if (price == null) return null;
      const note = extractPriceNote(text);
      const delivery = inferDeliveryMode(text);
      return {
        price,
        price_note: note,
  ...
  return {
    prices: priceDetails.map((d) => d.price),
    priceConfidence: priceDetails.map(() => 0.9),
    priceNotes: priceDetails.map((d) => d.price_note),
    priceNoteConfidence: priceDetails.map((d) => (d.price_note ? 0.75 : 0)),
    deliveryModes: priceDetails.map((d) => d.delivery_mode),
    deliveryConfidence: priceDetails.map((d) => d.delivery_confidence),
    inclusions,
    inclusionConfidence: inclusionConf
  };
}
