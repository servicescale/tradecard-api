...
function parseServicePanels($, base) {
  ...
  candidates.each((_, el) => {
    ...
    const title = norm(
      $p
        .find('[class*="title" i], [class*="name" i], h3, h2, h4')
        .first()
        .text()
    );
    if (title) svc.title = title;
...
async function collectCssAssets(entryUrls, baseForResolve, { maxFiles = 20, maxDepth = 2, onCss = null } = {}) {
  const queue = [];
...
async function parse(html, pageUrl, options = {}) {
  const $ = cheerio.load(html);
  const pageLanguage = ($('html').attr('lang') || '').trim() || null;
  const norm = (t) => (t || '').replace(/\s+/g, ' ').trim();
  const metaSink = options.meta || null;
  ...
  const cssAssets = await collectCssAssets(Array.from(new Set([...sheetHrefs, ...importsInline])), baseForResolve, { maxFiles: 20, maxDepth: 2, onCss: collectVars });
  ...
  const blockTextSample = (value, limit = 240) => {
    const text = norm(value);
    if (!text) return null;
    return text.length > limit ? `${text.slice(0, limit)}…` : text;
  };
  const resolveBlockUrls = (nodes) =>
    nodes
      .map((_, el) => resolveUrl($(el).attr('src') || $(el).attr('href'), baseForResolve))
      .get()
      .filter(Boolean);
  const blockSelector = ($el) => {
  ...
  $('body')
    ...
    .each((index, el) => {
      ...
      const headingsInBlock = [];
      ['h1','h2','h3','h4','h5','h6'].forEach((level) => {
        const texts = $el.find(level).map((_, h) => norm($(h).text())).get().filter(Boolean);
        if (texts.length) headingsInBlock.push({ level, text: texts[0] });
      });

      const blockImages = resolveBlockUrls($el.find('img'));
      const blockLinks = resolveBlockUrls($el.find('a[href]'));
      const blockText = blockTextSample($el.text());
      const paragraphs = $el.find('p').length;
      const lists = $el.find('ul,ol').length;
      const forms = $el.find('form').length;
      const buttons = $el.find('button,[role="button"]').length;

      pushBlock({
        order: index,
        tag,
        selector: blockSelector($el),
        role: $el.attr('role') || null,
        aria_label: $el.attr('aria-label') || null,
        text_sample: blockText,
        headings: headingsInBlock,
        image_count: blockImages.length,
        images: blockImages.slice(0, 8),
        link_count: blockLinks.length,
        links: blockLinks.slice(0, 8),
  ...
  const pickSummaryText = (candidates = []) => {
    let best = '';
  ...
  const topicCandidates = [
    ...
    ...headings.h2,
    ...headings.h3,
    ...(metaKeywords ? metaKeywords.split(',').map((t) => norm(t)) : [])
  ]
    .map((t) => norm(t))
    .filter(Boolean);
  const uniqueTopics = Array.from(new Set(topicCandidates.map((t) => t.toLowerCase())))
    .map((lower) => topicCandidates.find((t) => t.toLowerCase() === lower))
    .filter(Boolean)
    .slice(0, 10);

  const contentSections = (layout_blocks || [])
    .filter((block) => block && (block.text_sample || (block.headings && block.headings.length)))
    .slice(0, 12)
    .map((block) => ({
      order: block.order,
      heading: block.headings?.[0]?.text || null,
      text: block.text_sample || null,
      tag: block.tag || null,
      role: block.role || null,
      aria_label: block.aria_label || null,
      link_count: block.link_count || 0,
      image_count: block.image_count || 0,
      links: Array.isArray(block.links) ? block.links.slice(0, 3) : [],
      images: Array.isArray(block.images) ? block.images.slice(0, 3) : []
    }));

  const contentSummary = {
    title: title || ogTitle || null,
    description: metaDescription || ogDescription || null,
    primary_heading: headings.h1[0] || headings.h2[0] || null,
    summary: pickSummaryText([firstParagraphText, ...text_blocks]),
    key_topics: uniqueTopics,
