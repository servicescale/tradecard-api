...
async function resolveMVF({ raw = {}, tradecard = {}, allowKeys = new Set() } = {}) {
  ...
  // theme & trust signals
  if (theme.primary) put('theme_primary_color', theme.primary);
  else if (typeof meta['theme-color'] === 'string')
    put('theme_primary_color', meta['theme-color']);
  if (theme.accent) put('theme_accent_color', theme.accent);
  else if (typeof meta['msapplication-tilecolor'] === 'string')
    put('theme_accent_color', meta['msapplication-tilecolor']);
  for (const t of texts) {
