...
module.exports.resolveField = async function resolveField(
  ...
  const payload = {
    ...
    existing_fields: Object.fromEntries(
      Object.entries(fields || {}).slice(0, 50)
    ),
    ...
    testimonials: Array.isArray(raw.testimonials)
      ? raw.testimonials.slice(0, 15).map((t) => ({
        quote: t?.quote || '',
        reviewer: t?.reviewer || '',
        location: t?.location || '',
      ...
      : [],
    awards: Array.isArray(raw.awards) ? raw.awards.slice(0, 15) : [],
    profile_videos: Array.isArray(raw.profile_videos) ? raw.profile_videos.slice(0, 10) : [],
    social: Array.isArray(raw.social) ? raw.social.slice(0, 20) : [],
