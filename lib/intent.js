...
exports.applyIntent = async function applyIntent(tradecard = {}, { raw = {}, fullFrame = false, opts = {} } = {}) {
  ...
  Logger.debug('DETECTION', 'Detector summary', { set: Object.keys(byKey).length, byKey, rawCounts });
  ...
  if (!opts.noLLM) {
    ...
    const unresolvedKeys = allowKeys.filter((k) => k !== 'identity_abn' && (!fields[k] || fields[k] === '') && map[k]);
    ...
    Logger.debug('LLM_MERGE', 'LLM merge results', { proposed: Object.keys(proposals).length, accepted, rejected });
  } else {
    trace.push({ stage: 'llm_merge', skipped: true });
    Logger.debug('LLM_MERGE', 'LLM merge skipped', { reason: 'no_llm_option' });
  }
  const remaining = Array.from(allowSet).filter(k => !fields[k]);
  trace.push({ stage: 'unresolved', remaining });
  Logger.debug('COVERAGE', 'Unresolved fields', { remaining });
