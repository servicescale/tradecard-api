...
async function applyIntent(tradecard = {}, { raw = {}, infer = false, resolve = 'hybrid' } = {}) {
  ...
  if (sent_keys.size < 8) {
    const baseline = {
      identity_business_name: tradecard.business?.name,
      identity_website_url: tradecard.contacts?.website,
      identity_email: tradecard.contacts?.emails?.[0],
      identity_phone: tradecard.contacts?.phones?.[0],
      identity_logo_url: tradecard.assets?.logo,
      business_description: tradecard.business?.description,
      service_areas_csv: tradecard.service_areas,
    };

    const socials = Array.isArray(tradecard.social) ? tradecard.social : [];
    for (const plat of ['facebook', 'instagram', 'linkedin', 'twitter', 'youtube', 'tiktok', 'pinterest']) {
      const item = socials.find((s) => s.platform === plat);
      if (item && item.url) baseline[`social_links_${plat}`] = item.url;
    }

    const svc = tradecard.services?.list?.[0] || {};
    baseline.service_1_title = svc.title;
    baseline.service_1_description = svc.description;
    baseline.service_1_image_url = svc.image;
    baseline.service_1_cta_label = svc.cta_label;
    baseline.service_1_cta_link = svc.cta_link;
    ...
    for (const [key, val] of Object.entries(baseline)) {
      ...
      if (idx !== -1) dropped_empty.splice(idx, 1);
      audit.push({ key, status: 'ok', source: 'baseline', len: s.length });
    }
  }
  ...
  trace.push({ stage: 'intent_coverage', before: beforeCount, after: sent_keys.size, sample_sent: [...sent_keys].slice(0, 10) });
