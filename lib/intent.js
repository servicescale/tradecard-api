...
function hasJsonLdDescription(signals = {}) {
  return (signals.jsonld || []).some(
    (obj) => typeof obj?.description === 'string' && obj.description.trim()
  );
}

function hasLongTextBlock(signals = {}) {
  return (signals.text_blocks || []).some((block) => String(block).split(/\s+/).filter(Boolean).length >= 15);
}

function getMissingReason(key, signals = {}, clusters = {}) {
  switch (key) {
    case 'identity_email':
      return (signals.emails || []).length ? 'filtered_or_invalid' : 'no_email_signal';
    case 'identity_phone':
      return (signals.phones || []).length ? 'filtered_or_invalid' : 'no_phone_signal';
    case 'identity_website':
    case 'identity_website_url':
      return (signals.urls || []).length ? 'filtered_or_invalid' : 'no_url_signal';
    case 'identity_business_name':
      return (signals.jsonld || []).length ? 'no_name_in_jsonld' : 'no_business_name_signal';
    case 'identity_address':
      return (signals.addresses || []).length ? 'filtered_or_invalid' : 'no_address_signal';
    case 'identity_state':
      return (signals.geo_signals || []).length ? 'no_state_signal' : 'no_geo_signal';
    case 'identity_services': {
      const clustersList = clusters.services || [];
      if (!clustersList.length) return 'no_service_evidence';
      const eligible = clustersList.filter((cluster) => cluster.source_count >= 2);
      return eligible.length ? 'filtered_or_invalid' : 'insufficient_service_support';
    }
    case 'business_description': {
      const hasSource = hasJsonLdDescription(signals) || hasLongTextBlock(signals);
      if (!hasSource) return 'no_description_source';
      const eligible = (clusters.services || []).filter((cluster) => cluster.source_count >= 2);
      if (!eligible.length) return 'insufficient_service_support';
      return 'filtered_or_invalid';
    }
    case 'service_areas_csv': {
      const states = (signals.geo_signals || []).filter((signal) => ['NSW', 'QLD', 'VIC', 'WA', 'SA', 'TAS', 'ACT', 'NT'].includes(signal));
      return states.length ? 'filtered_or_invalid' : 'no_geo_signal';
    }
    default:
      return 'unresolved';
  }
...
exports.applyIntent = async function applyIntent(tradecard = {}, { raw = {}, fullFrame = false, opts = {} } = {}) {
  ...
  const { fields, signals, clusters } = resolveFromRaw(raw, { allowSet, map });
  ...
  const stateSignals = Array.isArray(signals.states)
    ? signals.states
    : (Array.isArray(signals.geo_signals) ? signals.geo_signals : []);
  const suburbSignals = Array.isArray(signals.suburbs) ? signals.suburbs : [];
  const serviceClusters = Array.isArray(clusters?.services) ? clusters.services : [];
  ...
  trace.push({
    stage: 'provenance',
    evidence: {
      identity_email: emailSignals.slice(0, 3),
      identity_phone: phoneSignals.slice(0, 3),
      identity_website_url: urlSignals.slice(0, 3),
      identity_address: addressSignals.slice(0, 2),
      identity_state: stateSignals.slice(0, 3),
      identity_suburb: suburbSignals.slice(0, 3),
      identity_services: serviceClusters.slice(0, 5).map((cluster) => ({
        label: cluster.signals?.[0]?.text || '',
        support_count: cluster.support_count,
        sources: cluster.sources
      })),
      business_description: {
        has_jsonld_description: hasJsonLdDescription(signals),
        long_text_blocks: (signals.text_blocks || []).filter((block) => String(block).split(/\s+/).filter(Boolean).length >= 15).length
      }
    }
  });
  ...
  const remaining = Object.keys(fields).filter((k) => !fields[k]);
  trace.push({
    stage: 'unresolved',
    remaining,
    details: remaining.map((key) => ({
      key,
      reason: getMissingReason(key, signals, clusters)
    }))
  });
