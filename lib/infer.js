...
async function inferTradecard(tradecard = {}) {
  ...
  Logger.debug('LLM', 'Business profile payload', { payload: userPayload });
  ...
  Logger.debug('LLM', 'Business profile response content', { content });
  if (!content) {
    return { _meta: { error: 'no_content', detail: data } };
  }
  const parsed = tryParseAssistant(content);
  Logger.debug('LLM', 'Business profile parsed response', { parsed });
