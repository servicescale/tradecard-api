...
function buildEvidencePayload(raw = {}, tradecard = {}) {
  const testimonials = Array.isArray(raw.testimonials)
    ? raw.testimonials.slice(0, 20).map((t) => ({
      quote: t?.quote || '',
      reviewer: t?.reviewer || '',
      location: t?.location || '',
      source: t?.source_label || t?.source_url || ''
    }))
    : [];
  const awards = Array.isArray(raw.awards) ? raw.awards.slice(0, 20) : [];
  const profileVideos = Array.isArray(raw.profile_videos) ? raw.profile_videos.slice(0, 10) : [];
  const social = Array.isArray(raw.social) ? raw.social.slice(0, 30) : [];
  const images = Array.isArray(raw.images)
    ? raw.images.slice(0, 40).map((img) => ({ src: img?.src || '', alt: img?.alt || '' }))
    : [];
  return {
